...
for required_file in llama-server.service ocr-server.service streamlit.service logged-exec.sh wait-llama.py wait-ocr.py; do
    if [[ ! -f "${SERVER_DIR}/${required_file}" ]]; then
        echo "File startup tidak ditemukan: ${SERVER_DIR}/${required_file}" >&2
        exit 1
    fi
done
...
ln -sfn "${SERVER_DIR}/llama-server.service" "${USER_SYSTEMD_DIR}/llama-server.service"
ln -sfn "${SERVER_DIR}/ocr-server.service" "${USER_SYSTEMD_DIR}/ocr-server.service"
ln -sfn "${SERVER_DIR}/streamlit.service" "${USER_SYSTEMD_DIR}/streamlit.service"
...
echo "Service document-agent sedang dinyalakan: ${PROJECT_DIR}"
